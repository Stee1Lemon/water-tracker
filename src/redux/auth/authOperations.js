...
import { dailyNorma, editUserInfo, logout, refreshUser, signin, signup, updateAvatar } from "../../redux/Api/api";
...
export const updateAvatarThunk = createAsyncThunk('user/avatar', async (photo, thunkApi) => {
    try {
...
export const editUserInfoThunk = createAsyncThunk('user/edit', async (credentials, thunkApi) => {
    try {
...
export const dailyNormaThunk = createAsyncThunk('auth/dailynorm', async (norm, thunkApi) => {
    try {
        const newNorm = Number(norm) * 1000;
        await dailyNorma(newNorm);
        return newNorm;
    } catch (err) {
        return thunkApi.rejectWithValue(err.message);
    }
